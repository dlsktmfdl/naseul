...
public class MemoDAO {
	...
	public boolean create(MemoDTO dto) {//insert할 데이터 갖고 올거
		boolean flag=false;
		...
		sql.append(" values(memo_seq.nextval,?,?,sysdate, (select nvl(max(grpno),0)+1 from memo)) ");
	...
	//조회수 증가 메소드
	public void upViewcnt(int memono) {//내가 읽고있는 데이터만 업데이트하겠다.
		Connection con=DBOpen.open();
		PreparedStatement pstmt=null;
		
		StringBuffer sql=new StringBuffer();//쿼리문 만들기.
		sql.append(" update memo ");	
